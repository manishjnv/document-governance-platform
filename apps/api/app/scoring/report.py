...
class ReportGenerator:
    ...
    async def generate_html_report(
        ...
        html += self._build_footer(audit_meta)
...
<body>
    <div class="container">
        <div class="header">
            <div class="brand">ScopeSense</div>
            <h1>{_esc(filename)}</h1>
...
"""
    ...
    def _build_footer(self, audit_meta: dict | None = None) -> str:
        """Build footer. audit_meta (Phase D): review provenance line."""
        audit_line = ""
        if audit_meta:
            models = sorted(set((audit_meta.get("models_used") or {}).values()))
            parts = [
                p
                for p in (
                    f"Models: {', '.join(models)}" if models else None,
                    f"Rules {audit_meta.get('rules_version')}"
                    if audit_meta.get("rules_version")
                    else None,
                    f"Doc SHA-256 {str(audit_meta.get('parsed_text_sha256'))[:12]}..."
                    if audit_meta.get("parsed_text_sha256")
                    else None,
                )
                if p
            ]
            if parts:
                audit_line = (
                    '<div class="footer-text">' + _esc(" · ".join(parts)) + "</div>"
                )
        return f"""
        </div>
        <div class="footer">
            <div class="footer-text">
                ScopeSense -- Document Governance Platform
            </div>
            <div class="footer-text">
                This report was automatically generated by ScopeSense.
            </div>
            {audit_line}
        </div>
    </div>
