...
Later the same session: scan kit on VVAH 1.4.0 with a weekly auto-update PR
(`scripts/update_vvah_kit.py`, `vvah-kit-update.yml`); CI rewritten to run the
full suite on Python 3.11 (`ci-cd.yml`, `requirements-dev.txt`); a whole
scanner run folder zipped now adds fix status checked against JUnit/Maven,
patches, scanner evidence and coverage to the register, deck and review page
(RCA #36-#39); "Keycloak Fix v1" copied into talk2maq's org. Live at
`c1286be`; details in the handoff and `CODE_REVIEW_MODULE_REFERENCE.md` §0.
...
logic change beyond confirm dialogs and the shared resize hook). **Deployed 2026-09-13 evening** (`06bd700`), live smoke clean; Lighthouse and the real-data
screenshot pass are still open. Status and judgement
...
Code Review heroes (`29dbd8c`). Live smoke 10/10 (and 18/18 on `22c88a9` earlier); Lighthouse after the screenshots, local build: `/` mobile 79 / desktop 100, `/product/mitre-coverage` 78 / 100 (see the WRAPUP handoff addendum); owner decision: all prod orgs stay free for now; PageSpeed NOT re-run. Later (`8ae053b`): prod api+worker switched to `nvidia/nemotron-3.5-lightning:free` via the VPS `.env` (compose passes the model vars since `d5c2a25`) because the paid chain is at the key limit; SOW screenshot added from a free-model review
...
**Content phase: first MITRE / Code Review authority posts, first /compare page, licence loose ends (2026-09-12, 7 commits `b48b510`…`23f5af3`, pushed + deployed at `4ddfd9f` later the same day):** blog model gained `pillar` + product-page `relatedUseCase` targets, keyboard-accessible pillar filter chips and byline on `/resources/blog`; four posts in `blog/data.ts` under `pendingReview: true` (noindex, out of sitemap) awaiting the user's read-through: `how-much-mitre-attack-does-your-siem-cover`, `reading-an-attack-navigator-layer`, `what-a-code-security-review-deliverable-should-contain`, `sow-vs-rfp-review-what-changes` (950-1025 words each, only plan §5 statistics, sources inline; author string stays "ScopeWise Team" — no named Person exists yet). `/compare/scopewise-vs-manual-review` (content-calendar row 17, no competitor named) with FAQPage + BreadcrumbList, in sitemap, header Resources dropdown and footer. Top-level `LICENSE` (proprietary, Foxfiber Retail LLP, Apache-2.0 vendor carve-out) and the Visa non-affiliation line on the `/codereview` shell; `kit.py` verified to ship vendor LICENSE/NOTICE. Build green; Lighthouse A11y 100 on blog index and compare page. Handoff: `docs/phases/summaries/SESSION_HANDOFF_2026_09_12_CONTENT_PHASE.md`.
