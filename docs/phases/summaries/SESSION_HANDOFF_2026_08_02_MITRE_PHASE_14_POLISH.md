...
| `b8d0e75` | List page: six-column table -> responsive card grid (big coverage % + plain-words "detects 20 of 908" line, labeled matrix bars instead of I/M/E, delta vs previous run, status helper lines, whole card clickable, no-match empty state) |
| `2698eda` | Export scopes + search: report/XLSX `scope` param (full / executive 1-3 page leadership PDF / coverage / gaps / assumptions per-tab cuts), Exec PDF + Full PDF buttons, per-tab PDF/Excel icons inline in the tab bar, clarified Navigator tooltip; XLSX Summary emphasis pass (visible 8496AD all-cell borders incl. merged rows, executive summary as 5 compact bold/italic pointers, bold centered KEY NUMBERS, centered short columns everywhere); on-page "is it covered?" search over technique ID/name, tactic, platform/asset, and rule names -> state-grouped drill-down panel (GET now enriches per-technique platforms). All five PDF scopes + scoped XLSX render-verified in prod |
| `51ea342` | Real-customer upload sample (docs/sample/MITRE_Sample/UploadSample: 109-rule Sentinel tracker + environment workbook, generator validates IDs and round-trips both files) **plus the ingest fix it exposed** — their headers ("Analytical Rule", "Datasource Used", "Query / Conditions") did not auto-detect, so this real workbook would have 422'd; synonyms widened for name/log_source/logic + inventory header-row variants. Verified by parsing both files inside the prod container |
