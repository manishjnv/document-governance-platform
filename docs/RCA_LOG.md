...
One entry per bug fixed in ANY session (live testing, review critique,
agent output, deploy smoke) — not only live incidents (rule tightened
2026-09-12). Format: symptom → root cause → fix (file:line) → prevention.
Every session appends here before it ends; check this log before touching
a file listed below — several of these are copy-pasted patterns that recur.
...
## 2026-09-11/12: Code Security Review module (VVAH import) — build, review and prod check

### 22. Exploit-chain step links pointed at the wrong findings (2026-09-11)

- **Symptom:** A chain declared as steps `[2, 4]` in VVAH's findings.json
  opened findings that were not the SQL injection / pickle pair it
  described, once the review was rendered in ScopeWise.
- **Root cause:** `app/codereview/ingest.py` re-sorts findings by severity
  then CVSS and renumbers them 1..N for display, but `chains[].steps` are
  indexes into VVAH's ORIGINAL order. The Sonnet builder carried the raw
  numbers through unchanged. Caught in the Phase 3 diff critique, before
  any test existed for it.
- **Fix:** `ingest.py::_from_findings_json` stamps `_pos` per finding;
  `_finalize` builds `pos_to_idx` after sorting and remaps every chain step
  (0-based only when a 0 appears, unmappable steps kept verbatim), then
  pops `_pos`. Test: `test_codereview_ingest.py::test_chain_steps_remap_to_sorted_idx`.
- **Prevention:** any field that references another record by position
  must be remapped in the same function that reorders — grep for
  `steps`, `canonical_idx`, `duplicates` whenever the sort key changes.

### 23. Highlight word lists drifted between web drawer and XLSX/PPTX (2026-09-12)

- **Symptom:** The XLSX/PPTX highlighted "pinned" as a fix word; the web
  drawer did not. Same finding, different emphasis per surface.
- **Root cause:** the risk/fix regexes were hand-copied into
  `FindingDrawer.tsx` and `report_xlsx.py` (PPTX imported the latter). A
  later edit added `pin(?:ned)?` to the Python copy only. Nothing checked
  the copies agreed.
- **Fix:** single source `app/codereview/highlight_words.json`;
  `report_xlsx.py:54-58` compiles from it; the web copy
  `[reviewId]/components/highlightWords.ts` is GENERATED by
  `scripts/generate_highlight_words.py`;
  `test_codereview_report.py::test_highlight_words_web_mirror_is_current`
  fails when the mirror is stale.
- **Prevention:** never keep the same list in two languages by hand —
  generate one side (same pattern as `prompts/` ← `generate_prompt_docs.py`)
  and add a mirror-freshness test.

### 24. PPTX "Fix in one line" showed `Use \`SELECT ...` (2026-09-12, prod deck check)

- **Symptom:** Remediation slide, finding 4: the one-line fix was cut to
  "Use `SELECT …" — the rest of the sentence vanished.
- **Root cause:** `report_xlsx.py::_SENTENCE_SPLIT` splits on
  `[.!?]` + space + capital. The recommendation "Use `SELECT ... FOR
  UPDATE` …" contains an ellipsis followed by "FOR", so the splitter
  ended the sentence inside the code span; `_first_sentences(…, 1)` then
  kept only the fragment. The drawer's TS splitter had the identical regex
  and the identical bug (bullets split mid-code).
- **Fix:** `(?<!\.\.)` lookbehind in both regexes —
  `report_xlsx.py:66` and `FindingDrawer.tsx` (`sentences` memo). Test:
  `test_codereview_report.py::test_sentences_keep_ellipsis_inside_code`.
- **Prevention:** sentence splitting over LLM prose must protect code
  spans and ellipses; when a text rule exists in Python AND TS, fix and
  test both in the same commit (see #23 for the generated-mirror route
  when the rule is data, not logic).

### 25. Synthetic fixture disagreed with real VVAH output on two fields (2026-09-11, golden scan)

- **Symptom:** First real `findings.json` (OWASP NodeGoat) rendered
  vulnerability type as `null` and the deck showed no VVAH version.
- **Root cause:** the synthetic sample assumed `vuln_class_label` is
  populated and `tool_version` lives in findings.json; real VVAH 1.3.0
  leaves `vuln_class_label` null and reports the version only in
  `run_manifest_*.json`.
- **Fix:** ingest falls back to `vuln_class`; the router copies the
  manifest `version` into `report.tool_version`. Golden test
  `test_real_nodegoat_golden`; VVAH 1.3.0 field lists pinned in
  `FINDING_FIELDS_1_3_0` / `FINAL_REPORT_FIELDS_1_3_0`, plus the
  every-field `acme_full_schema.json` fixture.
- **Prevention:** a synthetic fixture proves the UI, never the integration
