...
| `report_pptx.py` | 2026-08-14: PPTX briefing-deck builder (`build_pptx_export`) — client-presentation deck in the same purple/teal design system (cover, headline tiles, coverage-by-tactic bar chart, detection-quality doughnut, log-source table, top-5 fixes, roadmap with never-fired caveat, next steps). Deterministic from stored summary; reuses only the stored narrative's gap recommendations; python-pptx (new dep). Re-exported via `report.py`. |
