...
| `report_xlsx.py` | Phase 14h: the XLSX gap-register builder (`build_xlsx_export`, split out of `report.py`), openpyxl-native only (no xlsxwriter). 2026-08-14 (VFQ customer-deliverable review): deep-purple/teal restyle (zebra data rows, teal Summary section bands), a "Reference KQL (illustrative)" Technique Tracker column (per-Sentinel-table skeletons with never-fires/false-positive discipline comments; single-word via only), and "Log fields needed" reordered so the component matching the gap's chosen telemetry category leads. |
