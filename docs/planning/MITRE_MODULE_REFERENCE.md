...
| `connectors/` | Phase 13: SIEM pull package — `egress.py` (resolve-then-pin SSRF guard, the ONLY outbound-HTTP door; https on ports 443/8089 only, per-call host allowlists, redirects rejected, caps), `sentinel.py` (Entra client-credentials → alertRules → canonical template CSV; fixed Microsoft hosts, regex-validated IDs), `splunk.py` (2026-08-18: Bearer-token pull of saved searches via the Splunk REST API — first customer-supplied hostname, strict FQDN regex + the egress deny set; ES correlation-search annotations → MITRE tags; migration 037 widens the platform CHECK; awaiting a reachable customer Splunk env for first live use), `vault.py` (AES-256-GCM, AAD-bound to connection_id, `SIEM_CRED_KEY` env master key, key_version), `base.py` (dispatch + `PLATFORM_LABELS` + error taxonomy). Contract: `MITRE_SIEM_INTEGRATION_PLAN.md`. |
...
| `report_xlsx.py` | Phase 14h: the XLSX gap-register builder (`build_xlsx_export`, split out of `report.py`), openpyxl-native only (no xlsxwriter). 2026-08-14 (VFQ customer-deliverable review): deep-purple/teal restyle (zebra data rows, teal Summary section bands), a "Reference KQL (illustrative)" Technique Tracker column (per-Sentinel-table skeletons with never-fires/false-positive discipline comments; single-word via only; 2026-08-20 RCA #21: a via that isn't a known Sentinel table renders as a `<your_X_table>` placeholder, never as a fake table name), and "Log fields needed" reordered so the component matching the gap's chosen telemetry category leads, prefixed (RCA #21) with a how-to-read line naming the recommended source so the source-agnostic event-ID guidance can't be misread as contradicting the recommendation. |
| `report_pptx.py` | 2026-08-14: PPTX briefing-deck builder (`build_pptx_export`) — full 18-slide client deck in the purple/teal design system, mirroring the hand-built VFQ deck's structure: cover · agenda with how-to-read cards · 4 section dividers (giant numbers) · scope & inputs (stat tiles from `params.environment_lists`, N/A breakdown by reason class) · 5-step methodology with provenance counts + threat context · headline tiles · coverage-by-tactic bar chart with strongest/weakest callouts · detection-quality doughnut · log-source table · DERIVED "What's Working" and "Key Gaps" evidence cards (best/worst tactics, CJ-relevant gap count, zero-covered sources, never-fired share, unmapped rules — each gap card carries a "What to do" line) · top-5 fixes table · roadmap with outcomes + never-fired caveat · next steps with Effort·Impact tags · closing. Deterministic from stored summary; reuses only the stored narrative's gap recommendations; python-pptx dep. Re-exported via `report.py`. Data-gated slides (quality, log sources, strengths/gaps) skip themselves when their data is absent. |
...
override it in displays when present. **Source-suitability gate
(2026-08-20, RCA #21):** `_categories_provided` keeps ALL matching
sources per category (customer row order, not just the first) and
`_feasibility` runs each candidate through `_source_sees_technique`
before naming it as `via` — an IdP/SSO-class source (curated
`_IDP_SOURCE_KEYWORDS`: Okta/Entra/SSO/Duo/…; AD, domain controllers,
Kerberos etc. are deliberately NOT IdP-class) may satisfy the
"identity" category only for techniques whose platforms intersect
`_IDP_VISIBLE_PLATFORMS` (Identity Provider/SaaS/IaaS/Office).
Unsuitable candidates fall through to the next source, category, or
bucket; the gate applies to the ownable-tooling (mid) path too. Born
from a client catching "Okta-based detection for RDP" in a delivered
report — when adding a new log-source keyword rule, encode what the
source is *blind* to, not just its category. **Phase 11 threat weighting:**
...
| `GET /assessments/{id}/export.pptx` | any | 409 unless completed. 2026-08-14: PowerPoint briefing deck (`report_pptx.build_pptx_export`), StreamingResponse with the pptx MIME, filename `…-briefing-deck.pptx`. No scope param — always the full deck. |
...
| A12 | (pending commit) | 08-03 | Accuracy-plan phase A12: scope the MITRE trend auto-pick to the same customer (migration-free — `params.customer` inside the existing JSONB). Real prod symptom fixed: the report's "Trend vs your previous run" block picked the org's most-recent completed run filtered only by `org_id`, so an org running assessments for multiple end customers got customer A diffed against customer B's last run ("applicability changed: 292"-style garbage). New `_sanitize_customer()` (trim + 200-char cap, empty → key omitted) backs an optional `customer` upload Form field (`POST /assessments`) and a new "Customer / engagement" wizard input (upload path only — hidden for the Sentinel tab, whose value is auto-derived instead). Sentinel/SIEM paths auto-stamp `params.customer` inside the single shared `_create_assessment_from_pull` (covers all 3 pull callers — token-at-trigger, saved-connection, and the scheduled worker) from the saved connection's name, or the raw workspace when there's no saved connection — so scheduled re-runs group naturally with zero user input. Trend query (report.py's Phase 14e auto-pick) gained one `AND` clause: `params["customer"].astext.is_not_distinct_from(current.params.get("customer"))` — NULL-safe, so orgs that never set a customer keep the pre-A12 "most recent completed run" behavior unchanged. The explicit `/assessments/{id}/compare/{other_id}` endpoint (user-chosen baseline) and XLSX export (confirmed to never consume the auto-picked baseline) are untouched by design. List rows gain a `customer` field (same pattern as the existing `project_name` chip); assessment detail already returned the full `params` blob, so no detail-endpoint change was needed. No migration, no new tables, no new dependencies. 5 new targeted tests (3 trend-scoping in `test_mitre_report.py`: same-customer match, NULL↔NULL still matches, cross-customer run skipped; 1 in `test_mitre_siem.py`: workspace-derived stamp on the token-at-trigger path; 1 in `test_mitre_connections.py`: connection-name-derived stamp + 200-char truncation on the saved-connection path). Suite 879→884/7; `tsc --noEmit` clean. |
