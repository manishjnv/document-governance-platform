...
**Highlight word lists (2026-09-12, single source).** The risk/fix
keyword regexes used by the drawer, the XLSX and the PPTX live ONLY in
`apps/api/app/codereview/highlight_words.json`. `report_xlsx.py` loads it
at import (`report_pptx.py` imports the compiled regexes from there); the
web copy `apps/web/app/codereview/[reviewId]/components/highlightWords.ts`
is GENERATED by `python scripts/generate_highlight_words.py` and
