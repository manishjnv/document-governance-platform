...
| 2026-09-13 | **Deployed** (VPS build of `06bd700`, both hosts). Live smoke (Haiku): 30/30 routes 200 on scopewise.assessiq.in and scopesense.in, `app-theme` on app pages and login only (absent on `/`, Inter there), IBM Plex served from `/_next/static/media` with zero Google font requests, API healthy. Lighthouse not run (no Chrome on the dev box); real-data screenshot pass still open. |
